...
- **边界定稿的「startup tear」与 DB 回落的墙钟期限**（Task 8 实测）：上一条的 Suspense 模式有个衍生竞态——边界定稿（deferred value）会被 React 包进一次**无点名的 options 形态** `startViewTransition`，对 RiftDirector 的形态判定来说与真路由过渡不可区分 → 照常挂 rift-t1。DB 不可达时这一撕落在 goto 后数秒，会污染同时窗内的所有语法探针（实测：换肤 morph 负向用例被误判泄漏、T1 heldMs 被拖飘破墙钟上限）。取数侧用 `withDeadline(promise, 3000)`（src/lib/utils.ts）把失败回压到 3s 级，否则文档收尾 ≈11.2s 会击穿 e2e 的 30s goto；探针侧装前必须先过 `settleDeferredBoundaries`（tests/e2e/probes.ts，轮询骨架标记消失 + 400ms 宽限）。产品语义上「首屏边界定稿撕一下」暂接受（有内容变化、符合撕裂即标点）；若要豁免需在 director 区分 deferred commit 与真导航，React 无现成信号，未做。调 withDeadline 须同步调 settle 探针的超时。
