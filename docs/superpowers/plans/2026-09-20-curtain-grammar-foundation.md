...
- **边界定稿的「startup tear」与 DB 回落的墙钟期限**（Task 8 实测）：上一条的 Suspense 模式有个衍生竞态——边界定稿（deferred value）会被 React 包进一次**无点名的 options 形态** `startViewTransition`，对 RiftDirector 的形态判定来说与真路由过渡不可区分 → 照常挂 rift-t1。DB 不可达时这一撕落在 goto 后数秒，会污染同时窗内的所有语法探针（实测：换肤 morph 负向用例被误判泄漏、T1 heldMs 被拖飘破墙钟上限）。取数侧用 `withDeadline(promise, 3000)`（src/lib/utils.ts）把失败回压到 3s 级，否则文档收尾 ≈11.2s 会击穿 e2e 的 30s goto；探针侧装前必须先过 `settleDeferredBoundaries`（tests/e2e/probes.ts，轮询骨架标记消失 + 400ms 宽限）。产品语义上「首屏边界定稿撕一下」暂接受（有内容变化、符合撕裂即标点）；若要豁免需在 director 区分 deferred commit 与真导航，React 无现成信号，未做。调 withDeadline 须同步调 settle 探针的超时。**Task 8 评审批（I-1）定案修订**：页面层 race 被丢弃的那个 promise 仍会后台挂到 ≈11s 并长时间占住 in-flight 填充（Next dev 为此刷 stuck-state 类告警）；期限最终内化进 `listPublishedPosts` 的 `"use cache"` 边界内（3s，reject 不入缓存），页面只剩 try/catch，驱动层另有 connect/statement 封顶（src/lib/db/client.ts）。墙钟口径不变（≤3s 定胜负），settle 探针无需改。
- **hero 自撕是 CSS 不是 focusPull**（Task 8 实测定稿，取代计划正文）：Task 8 Step 1 写的「h1 进站 600ms 后整块跑一次 `focusPull`（复用 Task 5）」未采纳——JS 定时器驱动的入场与水合/路由时序赛跑，实测不如纯 CSS 稳定；最终实现是 globals.css 的 `hero-reveal` keyframes（对角 clip-path wipe-in + translate(-4%,2%)→none，300ms var(--ease-rift) both，动画挂载即跑一次，零 JS、天然 once-per-page-load，reduced-motion 入文件尾统一静态化块）。Step 5 提交文案里的「hero 拉焦自撕」同以此为准；`focusPull` 仍存活但只服务 ⌘K 的 T3 拉焦，不再兼任 hero 入场。
- **T9 评审批落地记录**（Task 10 Commit 1）：① `readingTime ?? 1` 两处（posts/[slug]、search）经评审确认**有意保留**——列内 null 仅存在于入库前窗口，渲染层 1-min 占位比「·  min」空档诚实，不改。② ActionButton 选择**落地**而非移除：json-render registry 内 ≈35 行，渲染 label + onClick 经 `useKernelService("actions").invoke(actionId, params)` 派既有 ui-action 通道，skin 走 `frameClass` 套 stitch/clean 框；错误态上屏。③ generateStaticParams 的 DB-down 出路比计划文本更窄：Cache Components 禁止空数组返回，而 `use cache` 边界的拒绝会被构建期缓存工作单元升为 prerender fatal（页面侧 try/catch 封不住，实测 Next 16.3.5）——最终双管齐下：取数函数边界内 catch 归空态（永不拒绝，代价：空列表以 hours 档入缓存靠 revalidate 自愈），generateStaticParams 失败时返回哨兵 slug（`_build-offline`，页面/metadata 对它零 DB 接触直接 notFound/兜底），DB 恢复后重新构建即恢复预渲染。
- **Task 10 验收状态表（spec §8，DB 黑洞环境下实测）**：
  | # | 验收项 | 状态 | 数字/证据 |
  |---|--------|------|----------|
  | 1 | `theme:check` 含 v2 字段 + validatePersona 单测 | ✅ met | accent/bg 4.70–15.51 全过 ≥4.5；rift/hum 值域 + stitch⇒tear sanity 门禁入 CLI；vitest 全量绿 |
  | 2 | E2E T1/T2/T3 + reduced 硬切无 rAF 实例 | ✅ met | collapse/command/fx.spec 绿；fx.spec T10-C：reduced 下零 rift 模块请求、导航 <800ms、无 canvas/幕环 |
  | 3 | 底噪审计：待机除 hum 外无其他动画 | ✅ met（口径注明） | noise-audit.spec：/zh 非 low 档 settle 5s 后 running 命名 CSS 动画 ⊆ {hum-tremor}；shader rAF 不在 getAnimations() API 面，由 #2 网络断言兜住 |
  | 4 | 篇章 persona 发布烘焙/投毒回退 | ⛔ 本计划外 | 见计划「后续」：Plan B 篇章人格，依赖 CMS persona 字段另出计划 |
  | 5 | 预算门禁：rift chunk ≤30KB gz 进 CI | ⚠️ partial | `size:check` 已建：rift 13.9KB ✓、idle-only ✓；首页 first-load **341.4KB vs 预算 170KB**（见下条偏差） |
