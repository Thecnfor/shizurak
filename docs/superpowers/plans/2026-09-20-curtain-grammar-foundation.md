...
- **Task 10 验收状态表（spec §8，DB 黑洞环境下实测）**：
  ...
  | 2 | E2E T1/T2/T3 + reduced 硬切无 rAF 实例 | ✅ met | collapse/command/fx.spec 绿；fx.spec T10-C：reduced 下零 rift 模块请求、稳态导航 min-of-second-pair <1200ms（T10 收尾批④：空载实测 138–173ms，全量并跑负载下 min p50≈1067/max≈1185，预算含机器负载非静默放宽）、无 canvas/幕环 |
